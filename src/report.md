...
1. Вывести время, часового пояса, в котором вы сейчас находитесь. Вывод следующей команды должен содержать `NTPSynchronized=yes`: \
  `timedatectl show`
![](6.png "time")
...
![](7_4_vim_search_replace.png "vim3")
...
* –d покажет системному администратору все процессы, за исключением так называемых «лидеров». Лидерство – это когда один процесс запускает другие. При запуске процесса с данным ключом пользователь не увидит первый сервис, но ему покажут зависимые от него.
...
* значение 0.0.0.0. - IP-адрес на локальной машине
